...
export const GET = route(async () => ok(await User.find().select('-passwordHash').sort({ createdAt: 1 })));

// Every account reaches every screen, so creating one is just name, title and login.
export const POST = route(async (request, { user }) => {
  const { name, role, username, password } = await body(request);
